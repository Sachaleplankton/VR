...
UAIScene* UAssimpFunctionLibrary::ImportScene(FString FileName, UObject* WorldContextObject, int Flags, bool DisableAutoSpaceChange)
{
	...
	if (!Scene)
	{
		UE_LOG(LogAssimp, Error, TEXT("Error importing scene: %s"), *FString(UTF8_TO_TCHAR(aiGetErrorString())));
		return nullptr;
	}

	// Construction de la scène (attention, ne pas encore remplir OwnedMeshes)
	UAIScene* NewScene = NewObject<UAIScene>();
	NewScene->FullFilePath = FileName;

	// Fusion manuelle des meshes
	int TotalVertices = 0;
	int TotalFaces = 0;
	...
	// Création d’un mesh fusionné
	aiMesh* CombinedMesh = new aiMesh();
	CombinedMesh->mNumVertices = TotalVertices;
	CombinedMesh->mVertices = new aiVector3D[TotalVertices];
	CombinedMesh->mNormals = new aiVector3D[TotalVertices];
	CombinedMesh->mTextureCoords[0] = new aiVector3D[TotalVertices];
	CombinedMesh->mNumUVComponents[0] = 2;

	CombinedMesh->mNumFaces = TotalFaces;
	CombinedMesh->mFaces = new aiFace[TotalFaces];
	...
	for (unsigned int i = 0; i < Scene->mNumMeshes; i++)
	{
		...
		{
			CombinedMesh->mVertices[VertexOffset + v] = SrcMesh->mVertices[v];
			if (SrcMesh->HasNormals())
				CombinedMesh->mNormals[VertexOffset + v] = SrcMesh->mNormals[v];
			if (SrcMesh->HasTextureCoords(0))
				CombinedMesh->mTextureCoords[0][VertexOffset + v] = SrcMesh->mTextureCoords[0][v];
		}
		...
		for (unsigned int f = 0; f < SrcMesh->mNumFaces; f++)
		{
			...
			aiFace& DstFace = CombinedMesh->mFaces[FaceOffset + f];

			DstFace.mNumIndices = SrcFace.mNumIndices;
			DstFace.mIndices = new unsigned int[DstFace.mNumIndices];

			for (unsigned int idx = 0; idx < DstFace.mNumIndices; idx++)
			{
				DstFace.mIndices[idx] = SrcFace.mIndices[idx] + VertexOffset;
			}
		}
	...
	// Création du wrapper UAIMesh
	UAIMesh* UnifiedMesh = NewObject<UAIMesh>();
	UnifiedMesh->Mesh = CombinedMesh;
	UnifiedMesh->StaticMesh = nullptr;
	NewScene->OwnedMeshes.Add(UnifiedMesh);
